...
module.exports.likeCard = (req, res) => {
  Card.findByIdAndUpdate(
    ...
    .then(card => res.send(card))
    .catch(err => {
      if (err.name === 'ValidationError') return res.status(VALIDATION_ERROR_CODE).send({ "message": "Переданы некорректные данные для постановки/снятии лайка." });
      if (err.name === 'CastError') return res.status(CAST_ERROR_CODE).send({ "message": "Передан несуществующий _id карточки." });
      res.status(500).send({ "message": "Произошла ошибка" })
    });
}
